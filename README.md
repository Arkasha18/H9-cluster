...
Для разовой проверки масштаба одометра и единиц мгновенного расхода на машине
используйте отдельный безопасный и ограниченный по частоте тег:

```bash
adb logcat -s H9TripTelemetry:I
```

Он выводит только числовые значения текущего счётчика поездки, преобразованного
расстояния, мгновенного расхода, скорости и monotonic timestamp — не чаще
одного раза в секунду. Перед публикацией фрагментов logcat убедитесь, что в них
нет других тегов с VIN, координатами или служебными данными автомобиля.
...
После остановки средний расход будет показан как `—`, а расстояние и
длительность останутся числовыми.
