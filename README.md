...
Для высокочастотного чтения оборотов и накопительного счётчика топлива ECM2
используется read-only подписка на `svc://fdbus_mcu_ipc`. Из изменения
счётчика приложение рассчитывает мгновенный расход: в литрах в час на месте и
в литрах на 100 км в движении. Необходимые ARM64-библиотеки из штатной системы
...
может обновляться реже, а мгновенный расход может быть недоступен.
...
секунду. Этот диагностический лог не выводит мгновенный расход. На голове
`car.basic.instant_fuel_consumption` отдаёт `NaN`, поэтому интерфейс получает
мгновенный расход отдельно из read-only счётчика ECM2.
