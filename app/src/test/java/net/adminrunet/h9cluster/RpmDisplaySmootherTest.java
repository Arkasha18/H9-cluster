...
public final class RpmDisplaySmootherTest {
    ...
    @Test
    public void sustainedIdleChangeEventuallyMovesToNewStableTenth() {
        RpmDisplaySmoother smoother = new RpmDisplaySmoother();
        long nowMs = 1_000L;
    ...
    @Test
    public void movingEngineRemainsResponsiveAndConverges() {
        RpmDisplaySmoother smoother = new RpmDisplaySmoother();
        long nowMs = 1_000L;
        smoother.update(900, 0, nowMs);

        float firstMoving = smoother.update(3000, 40, nowMs + 50L);
        assertTrue(firstMoving > 900.0f && firstMoving < 3000.0f);

        float displayed = firstMoving;
        for (int frame = 0; frame < 80; frame++) {
            nowMs += 50L;
            displayed = smoother.update(3000, 40, nowMs + 50L);
        }
        assertEquals(3000.0f, displayed, 1.0f);
    }
