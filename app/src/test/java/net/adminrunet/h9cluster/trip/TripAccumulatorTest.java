...
public final class TripAccumulatorTest {
    ...
    @Test
    public void validAverageRecoversAfterInvalidSample() {
        TripAccumulator accumulator = TripAccumulator.start(1_000L);
        ...
        assertEquals(10.0, summary.averageConsumptionLitersPer100Km, 0.0001);
        assertEquals(0.02, summary.fuelUsedLiters, 0.0001);
        assertTrue(summary.durationValid);
    }

    @Test
    public void usesVehicleTripAverageForFuelAndConsumption() {
        TripAccumulator accumulator = TripAccumulator.start(1_000L);
        accumulator.update(telemetry(
                1_000L, 60, 10.0, true, 8.0f, true));
        accumulator.update(telemetry(
                2_000L, 60, 10.5, true, 8.0f, true));
    ...
    public void doesNotUseInvalidTripAverageAsFuelFallback() {
        TripAccumulator accumulator = TripAccumulator.start(1_000L);
        accumulator.update(telemetry(
                1_000L, 60, 10.0, true, 0.0f, false));
        accumulator.update(telemetry(
    ...
    @Test
    public void usesLatestVehicleTripAverage() {
        TripAccumulator accumulator = TripAccumulator.start(0L);
        accumulator.update(telemetry(
                0L, 60, 10.0, true, 8.0f, true));
        accumulator.update(telemetry(
                30_000L, 60, 10.5, true, 8.0f, true));
