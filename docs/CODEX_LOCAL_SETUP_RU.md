...
- `02_yandex_map_transparency_mask.png` — область, где откалиброванная
  подложка полностью прозрачна для штатной карты;
...
использовать для вырезания готовой графики. Других обязательных ограничений
компоновки шаблон не задаёт. Полное описание находится в `README_RU.txt` внутри
каталога шаблона.
